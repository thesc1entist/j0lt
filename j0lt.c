...
size_t
readline(char* src, char** dest, ssize_t lim);
...
main(int argc, char** argv)
{
        ...
        if (read_file_into_mem(pathptr, (void**)&resolvlist, &szresolvlist) == false)
                err_exit("* file read error");
        if (filereadmode == false) {
                remove(pathptr);
                printf("- resolv list removed from %s\n", pathptr);
        }
        resolvlist[szresolvlist] = '\0';

        while (magnitude >= 1) {
                nread = 0;
                resolvptr = resolvlist;
                printf("+ current attack magnitude %d \n", magnitude);
                while (szresolvlist > 0 && (nread = readline(lineptr, &resolvptr, MAX_LINE_SZ_J0LT)) != 0) {
                        szresolvlist -= nread;
